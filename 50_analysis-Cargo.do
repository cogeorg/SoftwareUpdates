...
save versions_Cargo.dta, replace
}
...
merge m:1 repoid using repositories_Cargo.dta
	...
	// languages (managed by Cargo), i.e. this is the first cut
	keep if _merge == 3  
	drop _merge 
...
save covariates_Cargo-cuts.dta, replace
	keep repoid
	duplicates drop
...
{
insheet using repo_dependencies_Cargo-cuts-nodes-lcc.csv, delimiter(";") names clear
	sort repoid
	gen id_repo_new = _n  // based on LCC constructed *AFTER* covariate cuts were applied
save repo_dependencies_Cargo-cuts-repoid-lcc.dta, replace

insheet using repo_dependencies_Cargo-cuts-lcc.csv, delimiter(";") names clear
	rename v1 from_repo 
	rename v2 to_repo
save repo_dependencies_Cargo-cuts-lcc.dta, replace
}
...
//
{
use covariates_Cargo-cuts, clear	
	// APPLY NETWORK-BASED CUTS AS WELL
merge m:1 repoid using repo_dependencies_Cargo-cuts-repoid-lcc.dta 
	drop id_repo_new
	keep if _merge == 3
	drop _merge
...
save master_Cargo-cuts-lcc.dta, replace
outsheet using master_Cargo-cuts-lcc.csv, delimiter(";") replace
}
