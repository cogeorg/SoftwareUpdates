...
save versions_Cran.dta, replace
}
...
merge m:1 repoid using repositories_Cran.dta
	// repositories that exist in master but not in using are typically projects in other programming
	// languages (managed by Cran), i.e. this is the first cut
	keep if _merge == 3  
	drop _merge 
...
save covariates_Cran-cuts.dta, replace
	keep repoid
	duplicates drop
...
{
insheet using repo_dependencies_Cran-cuts-nodes-lcc.csv, delimiter(";") names clear
	sort repoid
	gen id_repo_new = _n  // based on LCC constructed *AFTER* covariate cuts were applied
save repo_dependencies_Cran-cuts-repoid-lcc.dta, replace

insheet using repo_dependencies_Cran-cuts-lcc.csv, delimiter(";") names clear
	rename v1 from_repo 
	rename v2 to_repo
save repo_dependencies_Cran-cuts-lcc.dta, replace
}
...
//
{
use covariates_Cran-cuts, clear	
	// APPLY NETWORK-BASED CUTS AS WELL
merge m:1 repoid using repo_dependencies_Cran-cuts-repoid-lcc.dta 
	drop id_repo_new
	keep if _merge == 3
	drop _merge
...
save master_Cran-cuts-lcc.dta, replace
outsheet using master_Cran-cuts-lcc.csv, delimiter(";") replace
}
