...
//
// ============================================================================
cd ~/Dropbox/Papers/10_WorkInProgress/SoftwareUpdates/Data/Cran/
...
//
// FIRST, TIME DATA
insheet using versions_Cran.csv, delimiter(";") names clear
...
save versions_balanced_Cran-cuts-lcc.dta, replace
	
// THEN, PREPARE DEPENDENCIES AS PANEL -- CAREFUL, CAN GET LARGE
use repo_dependencies_Cran-cuts-lcc.dta, clear
	expand 64
	bysort from_repo to_repo: gen id_year_month = _n
save repo_dependencies_Cran-cuts-lcc+date.dta, replace

// FINALLY, ACTUAL PANEL CONSTRUCTION
// (BASED ON DEPENDENCIES PANEL MERGED WITH VERSIONS PANEL)
use repo_dependencies_Cran-cuts-lcc+date.dta, clear
	rename to_repo repoid
merge m:1 repoid id_year_month using versions_balanced_Cran-cuts-lcc.dta
	keep if _merge == 3
	drop _merge
...
// 	scalar p95_num_dep_updates = r(p95)
	drop if num_dep_updates > 1

save num_dep_updates.dta, replace

// MERGE DEPENDENCY UPDATES PER REPOID
use versions_balanced_Cran-cuts-lcc.dta, clear
...
save balanced_panel_Cran-cuts-lcc.dta, replace
...
//
// MAIN: Lag = 1
use	balanced_panel_Cran-cuts-lcc.dta, clear  // NOTE: Not actually balanced since the last cut removed observations with num_dep_updates > p95
...
esttab model1 model2 model3 model4 model5, ///
    ...
    alignment(D{.}{.}{-1})		
esttab model1 model2 model3 model4 model5 using i_num_versions_Cran-cuts2-lcc-1.tex, ///
    replace ///
    label ///
    se ///
    b(%9.3f) ///
...
esttab model1 model2 model3 model4 model5 using i_num_versions_Cran-cuts2-lcc-0.tex, ///
    replace ///
    label ///
    se ///
    b(%9.3f) ///
...
esttab model1 model2 model3 model4 model5 using i_num_versions_Cran-cuts2-lcc-3m.tex, ///
    replace ///
    label ///
    se ///
    b(%9.3f) ///
...
//
use	balanced_panel_Cran-cuts-lcc.dta, clear
	gen is_updated = 0
	replace is_updated = 1 if num_versions > 0
	gen dep_is_updated = 0
	replace dep_is_updated = 1 if num_dep_updates > 0
	
	qui probit is_updated dep_is_updated 
	eststo model1

	qui probit is_updated num_deps
	eststo model2

	qui probit is_updated dep_is_updated num_deps 
	eststo model3
	
	qui probit is_updated i.id_year_month dep_is_updated num_deps 
	eststo model4
...
esttab model1 model2 model3 model4, ///
    ...
    alignment(D{.}{.}{-1}) ///
    drop(*year_month*)
esttab model1 model2 model3 model4 using e_num_versions_Cran-cuts2-lcc-0.tex, ///
    replace ///
    label ///
    se ///
    b(%9.3f) ///
