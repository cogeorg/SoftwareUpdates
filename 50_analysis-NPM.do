...
// 	su size, d 
// 	scalar p50_size = r(p50)  // small projects could be hobbyists, we focus on more professional developers
// 	drop if size < p50_size
//	
// 	drop if numstars <= 2
// 	drop if description == ""
// 	su numstars, d
...
// 	drop if numstars >= p95_numstars
	// apply additional cuts
	su size, d 
	scalar p75_size = r(p75)  // small projects could be hobbyists, we focus on more professional developers
	drop if size < p75_size
	
	drop if numstars <= 3311
	drop if description == ""
	su numstars, d
	scalar p95_numstars = r(p95)
	drop if numstars >= p95_numstars
...
{
insheet using repo_dependencies_NPM-cuts-nodes-lcc.csv, delimiter(";") names clear
	sort repoid
	gen id_repo_new = _n  // based on LCC constructed *AFTER* covariate cuts were applied
save repo_dependencies_NPM-cuts-repoid-lcc.dta, replace

insheet using repo_dependencies_NPM-cuts-lcc.csv, delimiter(";") names clear
	rename v1 from_repo 
	rename v2 to_repo
save repo_dependencies_NPM-cuts-lcc.dta, replace
}
...
//
{
use covariates_NPM-cuts, clear	
	// APPLY NETWORK-BASED CUTS AS WELL
merge m:1 repoid using repo_dependencies_NPM-cuts-repoid-lcc.dta 
	drop id_repo_new
	keep if _merge == 3
	drop _merge
...
save master_NPM-cuts-lcc.dta, replace
outsheet using master_NPM-cuts-lcc.csv, delimiter(";") replace
}
