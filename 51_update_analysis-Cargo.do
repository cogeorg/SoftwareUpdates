...
merge m:1 year_month using id_year_year_month.dta 
	...
	sort repoid id_year_month
save versions_unbalanced_Cargo-cuts-lcc.dta, replace
	
// THEN, ACTUAL PANEL CONSTRUCTION
// (BASED ON DEPENDENCIES PANEL MERGED WITH VERSIONS PANEL)
use repo_dependencies_Cargo-cuts-lcc+date.dta, clear
	rename to_repo repoid
merge m:1 repoid id_year_month using versions_unbalanced_Cargo-cuts-lcc.dta
	keep if _merge == 3
	drop _merge
...
save unbalanced_panel_Cargo-cuts-lcc.dta, replace

// ROBUSTNESS: unbalanced panel, no lag
use	unbalanced_panel_Cargo-cuts-lcc.dta, clear  // NOTE: Not actually balanced since the last cut removed observations with num_dep_updates > p95
...
esttab model1 model2 model3 model4 model5, ///
	...
	stats(N r2, fmt(%9.0fc %9.3f)) ///
    alignment(D{.}{.}{-1})		
esttab model1 model2 model3 model4 model5 using i_num_versions_Cargo-cuts2-lcc-unbalanced.tex, ///
    replace ///
    label ///
    se ///
    b(%9.3f) ///
...
save rev_balanced_panel_Cargo-cuts-lcc.dta, replace
...
//
// MAIN: Lag = 1
use	rev_balanced_panel_Cargo-cuts-lcc.dta, clear  // NOTE: Not actually balanced since the last cut removed observations with num_dep_updates > p95
...
esttab model1 model2 model3 model4 model5, ///
	...
	stats(N r2, fmt(%9.0fc %9.3f)) ///
    alignment(D{.}{.}{-1})		
esttab model1 model2 model3 model4 model5 using rev_i_num_versions_Cargo-cuts2-lcc-1.tex, ///
    replace ///
    label ///
    se ///
    b(%9.3f) ///
...
// EXTENSIVE MARGIN
//
use	rev_balanced_panel_Cargo-cuts-lcc.dta, clear
	gen is_updated = 0
	replace is_updated = 1 if num_versions > 0
	gen dep_is_updated = 0
	replace dep_is_updated = 1 if num_dep_updates > 0
...
esttab model1 model2 model3 model4, ///
    ...
    alignment(D{.}{.}{-1}) ///
    drop(*year_month*)
esttab model1 model2 model3 model4 using rev_e_num_versions_Cargo-cuts2-lcc-1.tex, ///
    replace ///
    label ///
    se ///
    b(%9.3f) ///
