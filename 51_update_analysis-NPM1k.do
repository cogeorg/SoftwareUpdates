...
save versions_balanced_NPM-cuts-lcc.dta, replace
	
// THEN, PREPARE DEPENDENCIES AS PANEL -- CAREFUL, CAN GET LARGE
use repo_dependencies_NPM-cuts-lcc.dta, clear
	expand 64
	bysort from_repo to_repo: gen id_year_month = _n
save repo_dependencies_NPM-cuts-lcc+date.dta, replace

// FINALLY, ACTUAL PANEL CONSTRUCTION
// (BASED ON DEPENDENCIES PANEL MERGED WITH VERSIONS PANEL)
use repo_dependencies_NPM-cuts-lcc+date.dta, clear
	rename to_repo repoid
merge m:1 repoid id_year_month using versions_balanced_NPM-cuts-lcc.dta
	keep if _merge == 3
	drop _merge
...
use versions_balanced_NPM-cuts-lcc.dta, clear
...
save balanced_panel_NPM-cuts-lcc.dta, replace
...
//
// MAIN: Lag = 1
use	balanced_panel_NPM-cuts-lcc.dta, clear  // NOTE: Not actually balanced since the last cut removed observations with num_dep_updates > p95
...
esttab model1 model2 model3 model4 model5, ///
    ...
    alignment(D{.}{.}{-1})		
esttab model1 model2 model3 model4 model5 using i_num_versions_NPM-cuts2-lcc-1.tex, ///
    replace ///
    label ///
    se ///
    b(%9.3f) ///
...
esttab model1 model2 model3 model4 model5 using i_num_versions_NPM-cuts2-lcc-0.tex, ///
    replace ///
    label ///
    se ///
    b(%9.3f) ///
...
esttab model1 model2 model3 model4 model5 using i_num_versions_NPM-cuts2-lcc-3m.tex, ///
    replace ///
    label ///
    se ///
    b(%9.3f) ///
...
//
use	balanced_panel_NPM-cuts-lcc.dta, clear
	gen is_updated = 0
...
esttab model1 model2 model3 model4, ///
    ...
    alignment(D{.}{.}{-1}) ///
    drop(*year_month*)
esttab model1 model2 model3 model4 using e_num_versions_NPM-cuts2-lcc-1.tex, ///
    replace ///
    label ///
    se ///
    b(%9.3f) ///
